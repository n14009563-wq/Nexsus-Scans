...
1. **Tracking** (`ChunkVisitTracker`) -- listens for players changing chunks (on join, and on any
   ...
   appended immediately as new chunks are found so nothing is lost on a crash).
2. **Scanning** (`ScanEngine` + `ChunkScanner`) -- on the configured interval (and once shortly
...
3. **Local output** (`TileWriter`) -- each scanned chunk becomes one 16x16 PNG at
...
4. **Base44 push** (`TilePyramid` + `WebhookPusher`) -- the flat per-chunk tiles from step 3 become
...
Produces `target/NexusScan-0.2.1.jar`. Requires Java 21 and network access to `repo.papermc.io` /
