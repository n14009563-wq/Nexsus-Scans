...
public final class IncrementalPyramid {
    ...
    /**
     * @param tilePixels the pixel width/height every tile is rendered at -- must match the size
     *                   of the finest-zoom images handed to {@link #update}, since coarser levels
     *                   are built by combining 2x2 children back down to this same fixed size.
     */
    public IncrementalPyramid(int minZoom, int maxZoom, int tilePixels) {
    ...
    public void update(int chunkX, int chunkZ, BufferedImage tile) {
        ...
        for (int zoom = maxZoom - 1; zoom >= minZoom; zoom--) {
            x = Math.floorDiv(x, 2);
            y = Math.floorDiv(y, 2);
            BufferedImage combined = TilePyramid.combineChildren(levels.get(zoom + 1), x, y, tilePixels);
