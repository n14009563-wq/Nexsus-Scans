...
public class ScanEngine {
    ...
    // Set once (asynchronously, well after construction) by NexusScanPlugin once
    // MinecraftAssetDownloader finishes -- null until then, meaning "render with flat colors".
    // volatile since it's written from a background thread and read from the main thread's tick
    // loop; a plain reference assignment is all that's needed, no locking.
    ...
    /** Called once (from a background thread) when {@link MinecraftAssetDownloader} finishes --
     *  {@code atlas} is null if the download/extraction failed, which simply keeps isometric
     *  rendering on flat colors, exactly as if this were never called at all. */
    public void setTextureAtlas(TextureAtlas atlas) {
        this.textureAtlas = atlas;
    }

    /** For status reporting (see NexusScanCommand) -- whether real block textures are loaded and
     *  in use right now, as opposed to still loading, disabled, or having failed to load. */
    public boolean hasTextureAtlas() {
    ...
    private static WebhookPusher.IsometricManifest isoManifestInfo(int pixelsPerBlock, int baselineY,
                                                                     int heightWindowBlocks) {
        IsometricRenderer.Bounds bounds = IsometricRenderer.computeBounds(pixelsPerBlock, heightWindowBlocks);
        return new WebhookPusher.IsometricManifest(pixelsPerBlock, baselineY, heightWindowBlocks,
    ...
    private void beginScanTicks(World world, List<Long> orderedKeys) {
        ...
        boolean isometric = config.isometricRenderingEnabled();

        // Legacy flat renderer settings (only read/used when rendering-mode: flat).
        int tilePixelSize = config.tilePixelSize();
        boolean reliefShadingEnabled = config.reliefShadingEnabled();
        double reliefShadingStrength = config.reliefShadingStrength();
        IncrementalPyramid flatPyramid = isometric ? null
                : new IncrementalPyramid(config.webhookMinZoom(), config.webhookMaxZoom(), tilePixelSize);

        // Isometric renderer settings (default; only read/used when rendering-mode: isometric).
        int isoPixelsPerBlock = config.isometricPixelsPerBlock();
        int isoBaselineY = config.isometricBaselineY();
        int isoHeightWindow = config.isometricHeightWindowBlocks();
        // No coarser overview pyramid for isometric tiles yet -- every finished tile is just
        // collected directly at "zoom 0", one tile per chunk, and pushed/drained the same way the
        // flat pyramid's changed-tile list is (see the push block below).
        List<TilePyramid.Tile> isoPending = isometric ? new ArrayList<>() : null;
        ...
        activeCycleTask = new BukkitRunnable() {
            ...
            public void run() {
                ...
                while (dispatchedThisTick < chunksPerTick && !queue.isEmpty()
                    ...
                    if (isometric) {
                        dispatchChunkIsometric(world, chunkX, chunkZ, isoPixelsPerBlock, isoBaselineY,
                                isoHeightWindow, unloadForced, finishedQueue, inFlight);
                    } else {
                        dispatchChunkFlat(world, chunkX, chunkZ, tilePixelSize, unloadForced, reliefShadingEnabled,
                                reliefShadingStrength, finishedQueue, inFlight);
                    }
                }
                ...
                while ((finished = finishedQueue.poll()) != null) {
                    ...
                    if (isometric) {
                        isoPending.add(new TilePyramid.Tile(0, finished.chunkX(), finished.chunkZ(), finished.image()));
                    } else {
                        flatPyramid.update(finished.chunkX(), finished.chunkZ(), finished.image());
                    }
                    sinceLastPush++;
                }

                boolean allWorkDone = queue.isEmpty() && inFlight.get() == 0;
                boolean hasPending = isometric ? !isoPending.isEmpty() : flatPyramid.hasPendingChanges();
                ...
                if ((allWorkDone || sinceLastPush >= pushBatchChunks) && hasPending) {
                    if (isometric) {
                        List<TilePyramid.Tile> delta = new ArrayList<>(isoPending);
                        isoPending.clear();
                        WebhookPusher.pushAsync(config, delta,
                                isoManifestInfo(isoPixelsPerBlock, isoBaselineY, isoHeightWindow), plugin.getLogger());
                    } else {
                        List<TilePyramid.Tile> delta = flatPyramid.drainChangedTiles();
                ...
                if (allWorkDone) {
                    ...
                    WebhookPusher.IsometricManifest manifestInfo = isometric
                            ? isoManifestInfo(isoPixelsPerBlock, isoBaselineY, isoHeightWindow) : null;
                    backgroundExecutor.submit(() -> TileWriter.writeManifest(config.outputDirectory(),
                            world.getName(), completedTiles, manifestInfo, plugin.getLogger()));
     ...
     * chunk, or a load failure), since {@link #beginScanTicks} waits for it to reach zero. Shared
     * by both renderers -- only what happens inside the background-thread render step differs, see
     * {@link #dispatchChunkIsometric} and {@link #dispatchChunkFlat}.
     */
    private void dispatchChunk(World world, int chunkX, int chunkZ, boolean unloadForced,
                                ConcurrentLinkedQueue<FinishedTile> finishedQueue, AtomicInteger inFlight,
                                java.util.function.Function<ChunkSnapshot, BufferedImage> render) {
        ...
        world.getChunkAtAsync(chunkX, chunkZ, false).whenComplete((chunk, error) -> {
            ...
            backgroundExecutor.submit(() -> {
                try {
                    BufferedImage image = render.apply(chunkSnapshot);
    ...
    private void dispatchChunkIsometric(World world, int chunkX, int chunkZ, int pixelsPerBlock, int baselineY,
                                         int heightWindowBlocks, boolean unloadForced,
                                         ConcurrentLinkedQueue<FinishedTile> finishedQueue, AtomicInteger inFlight) {
        // Read fresh per dispatch (not snapshotted once per scan): if the background texture
        // download finishes partway through a long scan, chunks dispatched from that point on
        // start getting real textures immediately rather than waiting for the next scan cycle.
        TextureAtlas atlas = textureAtlas;
        dispatchChunk(world, chunkX, chunkZ, unloadForced, finishedQueue, inFlight,
                snapshot -> IsometricRenderer.renderTile(snapshot, pixelsPerBlock, baselineY, heightWindowBlocks, atlas));
    }

    private void dispatchChunkFlat(World world, int chunkX, int chunkZ, int tilePixelSize, boolean unloadForced,
                                    boolean reliefShadingEnabled, double reliefShadingStrength,
                                    ConcurrentLinkedQueue<FinishedTile> finishedQueue, AtomicInteger inFlight) {
        dispatchChunk(world, chunkX, chunkZ, unloadForced, finishedQueue, inFlight,
                snapshot -> ChunkScanner.renderTile(snapshot, tilePixelSize, reliefShadingEnabled, reliefShadingStrength));
    }
}
