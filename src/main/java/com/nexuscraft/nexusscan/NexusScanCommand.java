...
public class NexusScanCommand implements CommandExecutor {
    ...
    private void handleStatus(CommandSender sender) {
        ...
        sender.sendMessage(ChatColor.GRAY + "Rendering mode: " + ChatColor.WHITE
                + (config.isometricRenderingEnabled() ? "isometric" : "flat"));
        if (config.isometricRenderingEnabled()) {
            sender.sendMessage(ChatColor.GRAY + "Real block textures: " + ChatColor.WHITE + describeTextureState());
        }
    ...
    /** See {@link ScanEngine#hasTextureAtlas()} -- covers the three real states an operator might
     *  need to distinguish: turned off in config, still downloading/extracting in the background
     *  (normal for the first minute or so after startup, or after a version change), or ready and
     *  actively being drawn on new tiles. */
    private String describeTextureState() {
        if (!config.isometricUseRealTextures()) return "disabled (isometric.use-real-textures: false)";
        return engine.hasTextureAtlas() ? "loaded, in use" : "loading (falling back to flat colors until ready)";
    }
